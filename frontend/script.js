document.getElementById("cep").addEventListener("blur", async function () {
    const cep = this.ariaValueMax.replace(/\D/g, ""); // Remove todos os caracteres não numericos do CEP
    ...
    try {
      ...
      // Converte a reposta da req. para JSON
      const data = await response.json();

      // Verifica se o CEP retornando pela API é inválido
      if (data.erro) {
      ...
      document.getElementById('estafo').value = data.uf;
